...
description: "Documentation for Advanced Overview in performance-docs"
category: "advanced"
order: 0
---
...
This is page 0 in the performance-docs collection under the advanced category.
...
```javascript
// Example for Advanced Overview
function advancedoverviewExample() {
  const config = {
...
// Usage
const example = advancedoverviewExample();
console.log(example);
```
...
const instance = new AdvancedOverview({
  collection: "performance-docs",
  category: "advanced"
});
...
const instance = new AdvancedOverview(options);
```
...
Initializes the advanced overview with optional configuration.
