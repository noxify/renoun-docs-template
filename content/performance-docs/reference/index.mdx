...
description: "Documentation for Reference Overview in performance-docs"
category: "reference"
order: 0
---
...
This is page 0 in the performance-docs collection under the reference category.
...
```javascript
// Example for Reference Overview
function referenceoverviewExample() {
  const config = {
...
// Usage
const example = referenceoverviewExample();
console.log(example);
```
...
const instance = new ReferenceOverview({
  collection: "performance-docs",
  category: "reference"
});
...
const instance = new ReferenceOverview(options);
```
...
Initializes the reference overview with optional configuration.
