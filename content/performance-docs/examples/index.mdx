...
description: "Documentation for Examples Overview in performance-docs"
category: "examples"
order: 0
---
...
This is page 0 in the performance-docs collection under the examples category.
...
```javascript
// Example for Examples Overview
function examplesoverviewExample() {
  const config = {
...
// Usage
const example = examplesoverviewExample();
console.log(example);
```
...
const instance = new ExamplesOverview({
  collection: "performance-docs",
  category: "examples"
});
...
const instance = new ExamplesOverview(options);
```
...
Initializes the examples overview with optional configuration.
