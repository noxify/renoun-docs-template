...
description: "Documentation for Guides Overview in performance-docs"
category: "guides"
order: 0
---
...
This is page 0 in the performance-docs collection under the guides category.
...
```javascript
// Example for Guides Overview
function guidesoverviewExample() {
  const config = {
...
// Usage
const example = guidesoverviewExample();
console.log(example);
```
...
```typescript
import { GuidesOverview } from './guides-overview';

const instance = new GuidesOverview({
  collection: "performance-docs",
  category: "guides"
});
...
const instance = new GuidesOverview(options);
```
...
Initializes the guides overview with optional configuration.
